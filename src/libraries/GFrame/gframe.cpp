/*************************************************** 
  This is a custom version of the Adafruit 1.8" SPI display driver.
  It adds ability to render out all writes in a single spi transaction 
  saving on overheads. Use the BeginDraw / EndDraw methods to use this feature.  
...
GFrame::GFrame(uint8_t cs, uint8_t rs, uint8_t sid,
...
GFrame::GFrame(uint8_t cs, uint8_t rs, uint8_t rst) : Adafruit_GFX(ST7735_TFTWIDTH, ST7735_TFTHEIGHT)
{
  _cs = cs;
  _rs = rs;
  _rst = rst;
...
inline void GFrame::spiwrite(uint8_t c)
{
...
void GFrame::writecommand(uint8_t c)
{
...
void GFrame::writedata(uint8_t c)
{
#ifdef SPI_HAS_TRANSACTION
  if (hwSPI)
    SPI.beginTransaction(spisettings);
...
void GFrame::commandList(const uint8_t *addr)
{
...
void GFrame::commonInit(const uint8_t *cmdList)
{
  ...
  { // Using hardware SPI
    SPI.begin();
#ifdef SPI_HAS_TRANSACTION
    spisettings = SPISettings(24000000L, MSBFIRST, SPI_MODE0);
#else
...
void GFrame::initB(void)
{
  commonInit(Bcmd);
}

// Initialization for ST7735B screens
void GFrame::initG(void)
{
  commonInit(Gcmd);
}

// Initialization for ST7735R screens (green or red tabs)
void GFrame::initR(uint8_t options)
{
  commonInit(Rcmd1);
...
void GFrame::setAddrWindow(uint8_t x0, uint8_t y0, uint8_t x1,
                           uint8_t y1)
{
...
void GFrame::pushColor(uint16_t color)
{
#ifdef SPI_HAS_TRANSACTION
  if (hwSPI)
    SPI.beginTransaction(spisettings);
...
void GFrame::drawPixel(int16_t x, int16_t y, uint16_t color)
{
  ...
  if (inTransaction)
  {
    buffer.Write(x, y, color, rotation);
    return;
  }
...
void GFrame::drawFastVLine(int16_t x, int16_t y, int16_t h,
                           uint16_t color)
{
  ...
  if (inTransaction)
  {
    while (h--)
    {
      buffer.Write(x, y + h, color, rotation);
    }
    return;
  }
...
void GFrame::drawFastHLine(int16_t x, int16_t y, int16_t w,
                           uint16_t color)
{
  if ((x >= _width) || (y >= _height))
    return;
  ...
  if (inTransaction)
  {
    while (w--)
    {
      buffer.Write(x + w, y, color, rotation);
    }
    return;
  }
...
void GFrame::fillScreen(uint16_t color)
{
  fillRect(0, 0, _width, _height, color);
}

void GFrame::fillRect(int16_t x, int16_t y, int16_t w, int16_t h,
                      uint16_t color)
{
  ...
  if (inTransaction) 
  {
    for (auto i = x; i < x + w; i++)
      for (auto j = y; j < y + h; j++)
        buffer.Write(i, j, color, rotation);
    return;
  }
...
void GFrame::setRotation(uint8_t m)
{
...
void GFrame::invertDisplay(boolean i)
{
  writecommand(i ? ST7735_INVON : ST7735_INVOFF);
}

void GFrame::beginDraw()
{
  buffer.Reset();
...
void GFrame::endDraw()
{
  this->setAddrWindow(0, 0, 160 - 1, 128 - 1);
  ...
  *csport &= ~cspinmask;
  ...
  for (auto y = 0; y < _height; y++)
    ...
    {
      auto val = buffer.Read(x, y, rotation);
      SPI.transfer16(val);
    }
  *csport |= cspinmask;
